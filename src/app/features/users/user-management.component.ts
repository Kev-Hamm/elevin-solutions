        ...
        <ng-container *ngIf="isAdmin">
          <div class="route-layout users-layout">
            <section class="surface-card invite-user-card">
              ...
              <h2>Invite user</h2>
              <p>A temporary password will be generated and emailed automatically. It is not displayed here.</p>
              ...
              <div class="sage-panel role-help-panel">
                <strong>Roles:</strong> Admin manages users; caseworker supports residents; operations manages placement flow; receptionist supports intake coordination.
              </div>
            </section>
            ...
            <section class="surface-card staff-users-card">
                ...
                <table class="admin-table">
                  ...
                  <tbody>
                    <tr *ngFor="let user of users">
                      ...
                      <td><button type="button" class="pill-button secondary small" (click)="resetPassword(user)" [disabled]="resettingUserId === user.id">{{ resettingUserId === user.id ? 'Sending reset...' : 'Reset password' }}</button></td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div class="mobile-record-list" *ngIf="!loadingUsers && users.length > 0">
                <article class="record-card staff-user-card" *ngFor="let user of users">
                  ...
                  <button type="button" class="pill-button secondary" (click)="resetPassword(user)" [disabled]="resettingUserId === user.id">{{ resettingUserId === user.id ? 'Sending reset...' : 'Reset password' }}</button>
                </article>
              </div>
            </section>
          </div>
...
export class UserManagementComponent implements OnInit {
  ...
  roleLabels: Record<string, string> = { admin: 'Admin', caseworker: 'Caseworker', operations: 'Operations', receptionist: 'Receptionist' };
  ...
  resetPassword(user: AdminUserRecord): void {
    ...
    this.adminUsersService.resetPassword(user.id).subscribe({
      ...
      error: (err) => { this.resettingUserId = null; this.actionError = err.error?.message || 'Unable to send reset email right now.'; },
    });
  }
